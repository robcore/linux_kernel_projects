...
define_global_rw_attr(powersave_bias);
define_global_ro_attr(min_sampling_rate);

static struct attribute * greenmax_attributes[] = {
...
static struct attribute_group greenmax_attr_group = {
	.attrs = greenmax_attributes,
	.name = "greenmax",
	};

static int cpufreq_greenmax_boost_task(void *data) {
	struct greenmax_info_s *this_greenmax;
	u64 now;
	struct cpufreq_policy *policy;
#ifndef CONFIG_CPU_FREQ_GOV_GREENMAX_TEGRA
	unsigned int cpu;
	...
	while (1) {
		...
		for_each_online_cpu(cpu){
			...
			if (policy->cur < cur_boost_freq) {
				start_boost = true;
				dprintk(GREENMAX_DEBUG_BOOST, "input boost cpu %d to %d\n", cpu, cur_boost_freq);
				target_freq(policy, this_greenmax, cur_boost_freq, this_greenmax->old_freq, CPUFREQ_RELATION_H);
				this_greenmax->prev_cpu_idle = get_cpu_idle_time_greenmax(cpu, &this_greenmax->prev_cpu_wall);
			}
			mutex_unlock(&this_greenmax->timer_mutex);
		...
		if (start_boost) {
			...
			dprintk(GREENMAX_DEBUG_BOOST, "%s %llu %llu\n", __func__, now, boost_end_time);
		}
	}

	pr_info("[greenmax]:" "%s boost_thread stopped\n", __func__);
	return 0;
}

static void greenmax_input_event(struct input_handle *handle, unsigned int type,
		unsigned int code, int value) {
	if (!is_suspended && touch_poke && type == EV_SYN && code == SYN_REPORT) {
		if (!input_boost_duration || !touch_poke_freq)
			return;

	if ((powersave_bias == POWERSAVE_BIAS_MAXLEVEL) ||
		(powersave_bias == POWERSAVE_BIAS_MINLEVEL)) {
		/* nothing to do */
			return;
...
static struct input_mediator_handler greenmax_input_mediator_handler = {
	.event = greenmax_input_event,
	};

#else

static unsigned int dbs_input_connect(struct input_handler *handler,
		struct input_dev *dev, const struct input_device_id *id) {
	struct input_handle *handle;
	int error;

	pr_info("[greenmax]:" "%s input connect to %s\n", __func__, dev->name);
	...
	err2: kfree(handle);
	pr_err("[greenmax]:" "%s faild to connect input handler %d\n", __func__, error);
	return error;
}
...
static struct input_handler dbs_input_handler = {
	...
	.name = "cpufreq_greenmax",
	.id_table = dbs_ids,
	};
#endif

#ifdef CONFIG_HAS_EARLYSUSPEND
static void greenmax_early_suspend(struct early_suspend *h)
{
	dprintk(GREENMAX_DEBUG_SUSPEND, "%s\n", __func__);
	ideal_freq = suspend_ideal_freq;
	is_suspended = true;
	greenmax_update_min_max_allcpus();
}

static void greenmax_late_resume(struct early_suspend *h)
{
	dprintk(GREENMAX_DEBUG_SUSPEND, "%s\n", __func__);
	ideal_freq = awake_ideal_freq;
	is_suspended = false;
...
static int cpufreq_governor_greenmax(struct cpufreq_policy *new_policy,
		unsigned int event) {
	...
	case CPUFREQ_GOV_START:
		...
		this_greenmax->cur_policy = new_policy;
		this_greenmax->cpu = cpu;

		greenmax_update_min_max(this_greenmax,new_policy);

		this_greenmax->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_greenmax->freq_table){
		...
		greenmax_powersave_bias_init_cpu(cpu);

		if (dbs_enable == 1) {
			if (!boost_task_alive) {
				boost_task = kthread_create (
						cpufreq_greenmax_boost_task,
						NULL,
						"greenmax_input_boost_task"
				);
				...
				pr_info("[greenmax]:" "%s input boost task created\n", __func__);
				sched_setscheduler_nocheck(boost_task, SCHED_FIFO, &param);
				get_task_struct(boost_task);
				boost_task_alive = true;
			}
#ifdef CONFIG_INPUT_MEDIATOR
			input_register_mediator_secondary(&greenmax_input_mediator_handler);
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
			register_early_suspend(&greenmax_early_suspend_handler);
#endif
		...
		if (!greenmax_powersave_bias_setspeed(
					this_greenmax->cur_policy,
					NULL,
					powersave_bias))
		 	dbs_timer_init(this_greenmax);
			mutex_unlock(&dbs_mutex);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_greenmax->cur_policy->cur > new_policy->max) {
			dprintk(GREENMAX_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_greenmax->cur_policy,
					new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_greenmax->cur_policy->cur < new_policy->min) {
			dprintk(GREENMAX_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_greenmax->cur_policy,
					new_policy->min, CPUFREQ_RELATION_L);
		}
		else if (powersave_bias != 0)
			greenmax_powersave_bias_setspeed(
				this_greenmax->cur_policy,
				policy,
				powersave_bias);
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!dbs_enable){
			...
			sysfs_remove_group(cpufreq_global_kobject, &greenmax_attr_group);
#ifdef CONFIG_INPUT_MEDIATOR
			input_unregister_mediator_secondary(&greenmax_input_mediator_handler);
#else
			input_unregister_handler(&dbs_input_handler);
#endif
#ifdef CONFIG_HAS_EARLYSUSPEND
			unregister_early_suspend(&greenmax_early_suspend_handler);
#endif
		}
...
static int __init cpufreq_greenmax_init(void) {
	unsigned int i;
	struct greenmax_info_s *this_greenmax;
	u64 wall;
	u64 idle_time;
	...
	greenmax_wq = alloc_workqueue("greenmax_wq", WQ_HIGHPRI, 0);
	if (!greenmax_wq) {
		printk(KERN_ERR "Failed to create greenmax_wq workqueue\n");
		return -EFAULT;
	}
	...
	{
		this_greenmax = &per_cpu(greenmax_info, i);
		this_greenmax->cur_policy = NULL;
		this_greenmax->ramp_dir = 0;
		this_greenmax->freq_change_time = 0;
		this_greenmax->cur_cpu_load = 0;
		mutex_init(&this_greenmax->timer_mutex);
	}

#ifdef CONFIG_HAS_EARLYSUSPEND
	greenmax_early_suspend_handler.suspend = greenmax_early_suspend;
	greenmax_early_suspend_handler.resume = greenmax_late_resume;
	greenmax_early_suspend_handler.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 100;
#endif

	return cpufreq_register_governor(&cpufreq_gov_greenmax);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_GREENMAX
fs_initcall(cpufreq_greenmax_init);
#else
module_init(cpufreq_greenmax_init);
#endif

static void __exit cpufreq_greenmax_exit(void) {
	unsigned int i;
	struct greenmax_info_s *this_greenmax;

	cpufreq_unregister_governor(&cpufreq_gov_greenmax);

	for_each_possible_cpu(i)
	{
		this_greenmax = &per_cpu(greenmax_info, i);
		mutex_destroy(&this_greenmax->timer_mutex);
	}
	destroy_workqueue(greenmax_wq);
}

module_exit(cpufreq_greenmax_exit);

MODULE_AUTHOR("Joe Maples <frap129@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_greenmax' - A Power Biased Governor");
MODULE_LICENSE("GPL");
